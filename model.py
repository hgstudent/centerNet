...
def model(c = 1, input_size=(512,512,3)):
  ...
  #Top left heatmap
  tl_hm = tf.keras.layers.Conv2D(256, (3,3), activation="relu", padding="same")(x)
  tl_hm = tf.keras.layers.Conv2D(c, (1,1), activation="sigmoid", padding="same", name="tl_hm")(tl_hm)

  #Top left embeddings
  tl_em = tf.keras.layers.Conv2D(256, (3,3), activation="relu", padding="same")(x)
  ...
  br_hm = tf.keras.layers.Conv2D(256, (3,3), activation="relu", padding="same")(x)
  br_hm = tf.keras.layers.Conv2D(c, (1,1), activation="sigmoid", padding="same", name="br_hm")(br_hm)

  #Bottom right embeddings
  br_em = tf.keras.layers.Conv2D(256, (3,3), activation="relu", padding="same")(x)
